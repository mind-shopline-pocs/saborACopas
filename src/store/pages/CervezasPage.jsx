    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.log(data.products.filter((product) => product.product_category === "Beer"))
                //console.log(data?.products?.filter((product) => product.product_category.toLoweCase() === "beer"));
               setCervezas(data?.products?.filter((product) => product.product_category === "Beer"));
        ...
        <div className='container'>
            <div className='row mt-5'>
                {cervezas.map((cerveza) => {
                    ...
                    return(
                        <div className="col-xl-3 col-lg-6 col-md-6" key={cerveza.id}>
                            <ProductCard props={cerveza} buttonState="Ver Cerveza" />
                        </div>
                    )
